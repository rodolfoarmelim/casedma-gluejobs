...
df_sot = glueContext.create_dynamic_frame.from_catalog(
    ...
    table_name="produto_credito_sot_andres"
).toDF()
...
df_spec = df_spec.groupBy("cc_num", "first_name", "last_name", "gender", "idade_cliente", "anomes") \
    .agg(sum("amt").cast(DecimalType(18,2)).alias("soma_valor_transacoes")
    , count("*").cast(IntegerType()).alias("quantidade_transacoes"))

# Criar coluna dt_carga com data e hora atual em America/Sao_Paulo
df_spec = df_spec.withColumn("dt_carga", from_utc_timestamp(lit(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
                                        , 'America/Sao_Paulo').cast(TimestampType()))
...
df_final = df_spec.select(
    ...
    col("quantidade_transacoes"),
    col("dt_carga"),
    col("anomes").alias("ano_mes_transacoes")
)

# Salvando a tabela final
df_final.write.insertInto("db_spec_banco_xis.produto_credito_spec_andres", overwrite=True)